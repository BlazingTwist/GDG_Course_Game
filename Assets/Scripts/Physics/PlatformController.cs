	...
	public class PlatformController : MonoBehaviour {
		...
		private void ApplyMoves(Queue<MoveInfo> moves) {
			while (moves.Count > 0) {
				...
				if (transformComponents.physicsController != null) {
					transformComponents.physicsController.Push(moveInfo.moveVector, -1);
				} else {
		...
		private readonly struct TransformComponents {
			...
			public readonly PhysicsMoveController physicsController;

			public TransformComponents(PhysicsMoveController physicsController) {
